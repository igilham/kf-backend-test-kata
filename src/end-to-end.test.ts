...
describe("end-to-end", () => {
  ...
  beforeEach(() => {
    scope = nock(getBaseURL());
    scope.get("/outages").reply(200, fakeOutages);
    scope.get("/site-info/kingfisher").reply(200, fakeSiteInfo);
    scope.post("/site-outages/kingfisher", expectedSiteOutages).reply(200, {
      message: "ok",
    });
  });

  test("main", async () => {
    await main(getBaseURL(), "fake-key", "kingfisher");
